...
public class MetadataUpgradeTest extends UpgradeTestBase {
  ...
  protected void postStage() throws Exception {
    ...
    searchResults = filterNonPurchaseEntities(metadataClient.searchMetadata(Id.Namespace.DEFAULT, "price",
                                                                            MetadataSearchTargetType.ALL));
    // should be at least 2 dataset: purchases + dataset: history
    Assert.assertTrue(2 < searchResults.size());

    searchResults = filterNonPurchaseEntities(metadataClient.searchMetadata(Id.Namespace.DEFAULT, "lastname:string",
                                                  MetadataSearchTargetType.ALL));
    // should be at least 1 dataset: history
    Assert.assertTrue(1 < searchResults.size());
  ...
  private Set<MetadataSearchResultRecord> filterNonPurchaseEntities(Set<MetadataSearchResultRecord> results) {
    Predicate<MetadataSearchResultRecord> appPredicate = new Predicate<MetadataSearchResultRecord>() {
      @Override
      public boolean apply(MetadataSearchResultRecord input) {
        ...
        if (entityId instanceof Id.DatasetInstance) {
          ...
          try {
            ...
            Assert.fail("Unable to retrieve CDAP version.");
          }
    ...
    return ImmutableSet.copyOf(Iterables.filter(results, appPredicate));
  }
}
